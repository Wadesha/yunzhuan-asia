...
(function (root) {
  ...
  // 枚举缓存里的 key（前缀过滤）。登录时已把该 slot 的所有 kv 拉进 _cache，故无需再查库
  SupabaseBackend.prototype.keys = function (prefix) {
    var ks = Object.keys(this._cache);
    if (!prefix) return ks;
    return ks.filter(function (k) { return k.indexOf(prefix) === 0; });
  };
